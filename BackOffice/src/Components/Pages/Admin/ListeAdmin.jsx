...
import { Table, Button, Modal, Form, Input, Upload, message, Popconfirm, Breadcrumb, Avatar } from 'antd';
import { UploadOutlined, PlusOutlined, EditOutlined, DeleteOutlined, UserOutlined } from '@ant-design/icons';
...
const ListeAdmins = () => {
  ...
  const handleUploadChange = (info) => {
    ...
    if (info.file.status === 'done') {
      ...
      if (currentAdmin) {
        setAdmins(admins.map(admin => admin._id === currentAdmin._id ? { ...admin, imageUrl } : admin));
      }
  ...
  const handleRemoveImage = () => {
    ...
    if (currentAdmin) {
      setAdmins(admins.map(admin => admin._id === currentAdmin._id ? { ...admin, imageUrl: null } : admin));
    }
  };
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <div className="flex items-center">
          {record.imageUrl ? (
            <img src={record.imageUrl} alt="Admin" className="w-10 h-10 rounded-full" />
          ) : (
            <Avatar style={{ backgroundColor: '#87d068' }} size={40} className="mr-2">
  ...
  return (
    <div className="p-4">
      ...
      <Button type="primary" onClick={() => setIsModalVisible(true)} className="mb-4" icon={<PlusOutlined />}>Ajouter un Admin</Button>
      <Table
        columns={columns}
        dataSource={admins}
        rowKey="_id"
      ...
      >
        <Form form={form} onFinish={handleFinish} layout="vertical">
          ...
          <Form.Item name="password" label="Mot de passe" rules={[{ required: currentAdmin ? false : true }]}>
            <Input.Password placeholder={currentAdmin ? "Laissez vide pour garder le mot de passe actuel" : "Entrez le mot de passe"} />
          </Form.Item>
          <Form.Item name="imageUrl" label="Image">
            ...
            {form.getFieldValue('imageUrl') && (
              <div className="flex items-center mt-2">
                <img src={form.getFieldValue('imageUrl')} alt="Admin" className="w-12 h-12 rounded-full" />
                <Button onClick={handleRemoveImage} type="link" danger className="ml-2">Supprimer</Button>
              </div>
            )}
          </Form.Item>
          <Form.Item>
