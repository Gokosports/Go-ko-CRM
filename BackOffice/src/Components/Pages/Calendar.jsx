...
const MyCalendar = () => {
  ...
  const dateCellRender = (value) => {
    const matchingEvents = events.filter((event) =>
      value.isSame(new Date(event.date_consultation), "day")
    );
    return matchingEvents.map((event, index) => (
      <div key={index}>
        ...
        >
          {new Date(event.date_consultation).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          })}{" "}
          - {event.motif_consultation}
        </div>
      </div>
    ));
  };
  ...
  return (
    <div className="w-full mx-auto mt-1 max-w-screen">
      ...
      <Modal
        title={
          selectedDate
            ? `Appointment : ${selectedDate.toLocaleDateString("en-EN", {
                weekday: "long",
                year: "numeric",
                month: "long",
                day: "numeric",
