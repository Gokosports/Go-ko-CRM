...
import { Breadcrumb, Table, Button, Modal, Form, Input, Select, message, Tabs, Popconfirm, Spin } from 'antd';
...
const ClientCommentsPage = () => {
    ...
    const getCommercialName = (commercialId) => {
        const commercial = commercials.find(commercial => commercial._id === commercialId);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text) => {
                const date = new Date(text);
                return isNaN(date.getTime()) ? 'Date Modification' : date.toLocaleString();
            },
        },
        {
            title: 'Actions',
        ...
        <div className="p-4 border rounded shadow-lg mt-4">
            ...
            >
                <Form form={form} layout="vertical" onFinish={handleSave}>
                    <Form.Item name="commentaire" label="Commentaire" rules={[{ required: true, message: 'Veuillez entrer un commentaire' }]}>
                        <Input.TextArea rows={4} />
                    </Form.Item>
                    <Form.Item label="Commercial">
                        <Input value={connectedCommercial} disabled />
                    </Form.Item>
                    <Form.Item name="commercialId" initialValue={decodedToken ? decodedToken.userId : ''} hidden>
                        <Input type="hidden" />
                    </Form.Item>
                    <Form.Item>
            ...
            <div className="mt-4">
                <Button onClick={() => navigate(`/client/${clientId}`)}>&lt; Retour</Button>
            </div>
        </div>
    );
};
