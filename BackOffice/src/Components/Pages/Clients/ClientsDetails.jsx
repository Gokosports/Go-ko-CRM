...
const ClientDetailsPage = () => {
    ...
    const fetchClient = async () => {
      try {
        ...
        const response = await axios.get(
          `https://go-ko-9qul.onrender.com/clients/${id}`,
          {
            headers: {
  ...
  const handleSave = async (values) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(`https://go-ko-9qul.onrender.com/clients/${id}`, values, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <div className="p-4 border rounded shadow-lg mt-4">
      ...
      <Tabs activeKey={activeTab} onChange={handleTabChange}>
        ...
        <TabPane tab="Commentaires" key="2">
          <div>{/* Ajoutez ici le contenu des commentaires */}</div>
        </TabPane>
      </Tabs>
      <Button
        type="primary"
