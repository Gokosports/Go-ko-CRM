...
import { Breadcrumb, Button, Table, Modal, Form, Input, Select, message, Upload, Image, Avatar, Tabs, Popconfirm } from 'antd';
import { EditOutlined, UploadOutlined, DeleteOutlined, UserSwitchOutlined, PlusOutlined } from '@ant-design/icons';
...
const CoachDetailsPage = () => {
    ...
    const handleSave = async (values) => {
        try {
            ...
            const updatedCoach = { ...coach, ...values, speciality: specialities.filter(s => values.speciality.includes(s._id)) };
    ...
    const handleAssign = async (values) => {
        try {
            ...
            setIsAssignModalVisible(false);
            setCoach({ ...coach, commercial: commercials.find(c => c._id === values.commercial) });
        } catch (error) {
    ...
    const data = [
        ...
        { key: '2', field: 'Prénom', value: isString(coach.prenom) ? coach.prenom : 'N/A' },
        { key: '3', field: 'Email', value: isString(coach.email) ? coach.email : 'N/A' },
        { key: '4', field: 'Téléphone', value: isString(coach.phone) ? coach.phone : 'N/A' },
        { key: '5', field: 'Âge', value: isString(coach.age) ? coach.age : 'N/A' },
        { key: '6', field: 'Sexe', value: isString(coach.sex) ? coach.sex : 'N/A' },
        { key: '7', field: 'Spécialité', value: Array.isArray(coach.speciality) ? coach.speciality.map(s => s.nom).join(', ') : 'N/A' },
        { key: '8', field: 'Ville', value: isString(coach.ville) ? coach.ville : 'N/A' },
        { key: '10', field: 'Commercial', value: coach.commercial ? `${coach.commercial.prenom} ${coach.commercial.nom}` : 'Non affecté' },
    ];
    ...
    const commentColumns = [
        ...
        },
        {
            ...
            key: 'commercialId',
            render: (text) => {
                const commercial = commercials.find(commercial => commercial._id === text);
                return commercial ? `${commercial.nom} ${commercial.prenom}` : 'Commercial inconnu';
            },
        },
        {
            title: 'Date',
            ...
            render: (text, record) => (
                <span key={record._id}>
                    <Button
                        ...
                        style={{ marginRight: 8, backgroundColor: 'white', borderColor: 'green', color: 'green' }}
                    />
                    <Popconfirm
                        title="Êtes-vous sûr de vouloir supprimer ce commentaire ?"
        ...
        <div className="p-4 border rounded shadow-lg mt-4">
            ...
            <Tabs activeKey={activeTab} onChange={handleTabChange}>
                <TabPane tab="Informations" key="1">
                    <div className="text-center mb-4">
                        {coach.imageUrl ? (
                            <img src={coach.imageUrl} alt="Coach" className="w-24 h-24 rounded-full mx-auto" />
                        ) : (
                ...
                <TabPane tab="Commentaires" key="2">
                    ...
                    <Table
                        ...
                        dataSource={comments.map(comment => ({ ...comment, key: comment._id }))}
                        rowKey="_id"
                        className="mt-4"
                        ...
                        <Form.Item label="Image">
                            ...
                            {coach.imageUrl && (
                                <div className="flex items-center mt-2">
                                    <Image src={coach.imageUrl} alt="Uploaded Image" width={50} height={50} className="mr-2" />
            ...
            >
                ...
                >
                    ...
                    <Form.Item>
                        <Button type="primary" htmlType="submit">Affecter</Button>
