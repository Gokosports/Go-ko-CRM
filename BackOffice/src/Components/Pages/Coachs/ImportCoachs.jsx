...
const ExcelImport = ({ onImportSuccess = () => {} }) => {
    ...
    const handleTransfer = async () => {
        ...
        try {
            ...
            if (uniqueData.length !== validData.length) {
                message.error('Certaines entrées ont des e-mails en double ou manquants');
                return;
            }
            ...
            const dataWithSpecialityIds = await Promise.all(uniqueData.map(async coach => {
                const specialities = coach.speciality ? await Promise.all(coach.speciality.split(',').map(async speciality => {
                    ...
                    if (specialityMap[lowerCaseSpeciality]) {
                        ...
                        const newSpecialityId = await addNewSpeciality(trimmedSpeciality, specialitySet);
                        console.log(`Nouvelle spécialité ajoutée: ${trimmedSpeciality} avec ID: ${newSpecialityId}`);
            ...
            const cleanedData = dataWithSpecialityIds.map(coach => {
                coach.speciality = coach.speciality.filter(speciality => speciality !== null && speciality !== undefined);
                return coach;
            });
            ...
            const response = await axios.post('https://go-ko.onrender.com/coaches/import', cleanedData, {
                headers: {
            ...
            const errorMessage = error.response && error.response.data && error.response.data.message ? error.response.data.message : 'Erreur inconnue';
    ...
    const columns = fileData && fileData.length > 0
        ? Object.keys(fileData[0]).map(key => ({ title: key, dataIndex: key, key }))
        : [];
                        ...
                        renderItem={item => (
                            <List.Item
                                actions={[
                                    ...
                                    <Button type="link" onClick={() => setIsModalVisible(true)}>Aperçu</Button>
                                ]}
                            >
                                <List.Item.Meta
