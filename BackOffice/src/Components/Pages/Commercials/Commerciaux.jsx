...
const CommercialsPage = () => {
  ...
  const handleEditCommercial = (commercial) => {
    ...
    setUploadedFileName(
      commercial.image ? commercial.image.split("/").pop() : ""
    );
  ...
  const handleDeleteCommercial = async (commercialId) => {
    try {
      await axios.delete(
        `https://go-ko-9qul.onrender.com/commercials/${commercialId}`
      );
  ...
  const handleSave = async (values) => {
    ...
    try {
      if (isEditing) {
        await axios.put(
          `https://go-ko-9qul.onrender.com/commercials/${currentCommercial._id}`,
          values
        );
        message.success("Commercial mis à jour avec succès");
      } else {
        await axios.post("https://go-ko-9qul.onrender.com/commercials", values);
