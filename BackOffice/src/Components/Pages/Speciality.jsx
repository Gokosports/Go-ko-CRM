...
const SpecialityDashboard = () => {
  ...
  const deleteSpeciality = async (id) => {
    try {
      await axios.delete(`https://go-ko-9qul.onrender.com/speciality/${id}`);
      setSpecialities(
        specialities.filter((speciality) => speciality._id !== id)
      );
  ...
  const handleAddNew = async (values) => {
    const existingSpeciality = specialities.find(
      (speciality) => speciality.nom.toLowerCase() === values.nom.toLowerCase()
    );
  ...
  const handleEditSave = async (values) => {
    const isDuplicate = specialities.some(
      (speciality) =>
    ...
    try {
      const response = await axios.put(
        `https://go-ko-9qul.onrender.com/speciality/${currentSpeciality._id}`,
        values
      );
      setSpecialities(
          ...
          >
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: "Veuillez entrer le nom de la spécialité!",
                },
              ]}
            >
              <Input />
